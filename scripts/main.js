...
function updateDashboard() {
  ...
  // Update x-axis labels
    kpiChart.data.labels = ['Tid', 'Score']; // Clear existing labels
    kpiChart.update();
...
function openTaskSelectionModal() {
    if (gameState.currentTask) {
        ...
        document.getElementById('activeWarn').addEventListener('click', closeModal); // Use imported function
        return;
    }
...
<td\><button class\="commit\-task\-btn modern\-btn" data\-idx\="</span>{index}">Forpligt</button></td>
  ...
  `;
    ...
    document.getElementById('closeTaskModal').addEventListener('click', closeModal); //Use imported function
...
function showStepChoices(step) {
    ...
    const choiceHandler = (choice, isAdvanced) => {
        ...
        applyChoice(appliedChoice);
        gameState.choiceHistory[gameState.currentStepIndex] = { title: choice.label, advanced: isAdvanced };
